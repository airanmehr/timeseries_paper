\begin{abstract}
  ...
  While being more predictive than static data analysis, majority of the EE
  studies are constrained by the limited time span since onset of
  selection, depending upon the generation time of the organism.
   This constraint curbs the power adaptation studies, as the population can only 
   be evolve-and-resequenced for a small number of generations relative to the
  fixation-time of the favored allele. Moreover, coverage in
  pool-sequenced experiments varies across replicates and time points for every 
  variant. As a result, ascertainment bias in measuring population frequency is 
  heterogeneous across different measurements of a single allele.
  ...
  byproduct of the statistical testing, while being orders of magnitude
  faster. Finally, we applied the \comale\
  statistic to \datadm, and discovered enrichment of genes related to ``response 
  to heat", ``cold acclimation" and  ``defense response to bacterium" ontologies, 
  many of which express Heat Shock Proteins (HSPs).
\end{abstract}