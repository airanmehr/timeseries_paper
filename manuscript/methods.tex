...
(Suppl.~Fig.~\ref{fig:stateConditional},\ref{fig:trajectoryReal}).  In
pool-seq datasets, the sequencing depth at a site varies for different
replicates, and different time samples
(Suppl.~Fig.~\ref{fig:depthHetero}). Therefore, filtering low coverage
variants can potentially discard useful information.  For instance, in
analyzing \datadm~\cite{orozco2012adaptation}, by setting minimum read
...
11,232 (Suppl. Fig.~\ref{fig:depth}). To account for this
...
\begin{table}
	...
	\begin{tabular}{l|c}
		...
		$h>1$ &overdominance
	\end{tabular}
		\caption{\bf Overdominance parameter values and their 
		implications.}
\end{table}
...
50Kbp region (Fig.~\ref{fig:ee}A). The remaining parameters were
