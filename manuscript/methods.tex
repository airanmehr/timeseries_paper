...
likelihood. The simplest form of the modified likelihood ratio test statistic  
...
(Suppl.~Fig.~\ref{fig:stateConditional},\ref{fig:trajectoryReal}).  In pool-seq 
datasets,
the sequencing depth at a site varies for different replicates, and different
...
maximize likelihood. The modified likelihood ratio statistic for
...
negative modified likelihood ratio values, but are still informative about
...
according to $\Xc^2$. However, Feder \emph{et
...
\item{\bf Forward Simulation.} We used forward simulations for
  evolving populations under selection. We also consider selection regimes  
  which the favored allele is be chosen merely from standing variation (not 
  \emph{de novo} mutations). Given initial diploid population, position of
  the site under selection, selection strength $s$, number of
  replicates $R=3$, recombination rate $r=2\times10^{-8}$ and sampling
  times $\Tc=\{0,10,20,30,40,50\}$, \texttt{simuPop}~\cite{peng2005simupop} was 
  used to
...
specifically if sampling is started long after onset of selection. While SFS 
based techniques expected to perform poorly in controlled EEs, their power is 
highest near fixation. To simulate these scenarios,
