...
Also, chromosomes 3L and 3R 
appear to have smaller population size~\ref{fig:estimateN}-D, $\hN=200,150$, 
respectively.
Next, we found maximum likelihood estimates of $s$, and computed the test 
statistic $\Hc*$ with $N=250$  on sliding windows of size of 500 SNPs 
and step size of 100  variants over the genome.
...
FDR$\le0.05$, which form 5 contiguous interval covering 
2,829 polymorphic sites. We then selected the 174 significant variants with FDR 
$\le0.01$\% within selected regions using single locus hypothesis testing.
To compute $p$-values of 
...
equal of test statistic(see~\ref{fig:null-alt}).
The selected 174 variants fall within 32 
genes~\ref{tab:genes}. 
\ignore{
We also performed GO
enrichment and found 
enriched GO terms including XXXX, XXXX ~\ref{tab:go-reg}.}
...
\ignore{However, it has been shown 
