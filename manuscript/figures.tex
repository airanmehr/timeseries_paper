...
\caption{{\bf Evolve and Resequence Selection Experiments on \dmel.}
  ...
  large population size population ($N_e=10^{6}$). Then, a population 
  of N 
  individuals is created and multiple replicates of the population 
  evolved and resequenced to collect time-series genomic dataset. For 
  sequencing, $m$ individuals are randomly selected to be 
  sequenced with coverage of $\lambda$.
   (B) Graphical model 
  showing dependence of the random
  variables in the single-locus model used to compute \comale\
  statistics. Only shaded variables are observed. Observer variables, 
  $c,d$, derived allele read count and coverage are shaded. The 
  variables $\nu,y,\lambda$ represent allele frequency, sampled allele 
  frequency, and sequencing coverage, respectively. (C) Mean and the
