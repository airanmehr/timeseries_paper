...
\begin{figure}[H]
	...
	\caption{{\bf Estimating N on simulated and real data.}
		Average and 95\% confidence interval if likelihood of simulated data 
		with $N=200$ (A), $N=600$(B), and $N=1000$ individuals, over 100 
		simulations, shows that estimator is unbiased.
		Chromosome-wise (D) and genome-wide (E) estimation of population size 
		for \datadm. Chromosome 3R fits population size of 150, while 
		genome-wide population size is 250. 
		(F) Despite large census population size 
		($10^6-10^7$~\cite{burke2014standing}), Yeast dataset exhibits much 
		smaller ($\hN=2000$) populations size.
	}
	\label{fig:estimateN}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{{\bf Scan of composite statistic on \datadm.}
		Manhattan plot of scan for $\Hc^*$ statistic over the genome. 
		The dashed line represents cutoff for  genome-wide FDR$\le0.05$,
		selecting 16 regions (A). Trajectories of the top 174 variants shown in 
		panel (B).
	}
...
\begin{figure}[H]
	...
	of scan for testing  overdominant selection (A).
		Significant variants with FDR $\le 0.01$ are denoted 
		in red, and their trajectories are depicted in panel (B).}
