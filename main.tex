...
with 
continues culture, fixed population size. 
...
Selective sweep is a process which under infinite site assumption
its dynamic is determined selection strength,
population size, recombination rate and the underlying genealogy at the onset 
of 
selection. 
As a result, the change in different population statistics, including 
{site allele frequencies (AF), SFS and 
	LD, can be attributed to the selection event.
...
change, respectively. Therefore, ``identification" of a selection of 
...
between the first and last sampled generations in the time-series data. Given a 
fixed amount of time for an study, the larger generation times of the organism 
under study, the smaller number of generations we can evolve and re sequence. 
In other words, for not strong enough-selectios, STS will be significantly 
smaller than fixation time. As a result, tests for detecting selection on 
static data, 
which work the best when the beneficial allele is close to fixation, fail to 
detect ``incomplete-sweep" in short-term experimental evolutions. 

Moreover, in controlled experimental evolution experiments, populations are
evolved and 
inbred under selective pressure. This scenario in which population 
size 
significantly reduces from large number of wild type (e.g.D. melanogaster 
...
drift quite strong. Also, most of conventional tests of selection can not 
distinct between demographic and selection.
As a result, strong drift and bottleneck confounds allele-frequency and SFS 
data respectively which makes difficult to differentiate selection from drift 
and demography.

\section{Results}

\paragraph{Allele Frequency Based Tests.}

However, Brownian motion provides a poor 
approximation for the distribution of increments of allele frequencies 
when $\nu_0$ is not close to $0.5$, or when sampling in time is sparse i.e., 
$t>1$, (Figure \ref{fig:markov}). At the 
start of selection, 
site frequencies 
are distributed according to SFS of a neutral population, and 
thus most of the 
sites are at low frequencies (Figure \ref{fig:afs}). Thus if the selecting 
site 
is 
chosen randomly 
with high probability its frequency will be far from 0.5. 
Moreover, in many 
...
\nu_{t}^j (1-\nu_{t})^{2N-j}  
\eeq
where $N$ is the census population size, and $P_{i,j}$ denotes probability of 
...
Alternatively, parametric models for single-locus selection can be 
...
In fact, it has been shown that the power of identifying selection can be 
improved by exploiting the mechanisms that selection changes allele 
frequencies \cite{nielsen2005genomic} \footnote{Basically what they did is 
quite similar to ours. First they came up with a neutrality test, then they 
extend it by a parametric formulation which they can estimate $s$ and 
compute likelihood.}.
...
each site by a simple regression (see Section \ref{sec:regression} 
for details).

\paragraph{SFS based tests}
We also considered SFS based tests for dertecting selection. We looked  a nim 
of existing tests that compute weighted sum  of scaled SFS.
As these thests work only on one time points, we extend them to handle 
...
which half are neutral and the rest are under selection. Then prediction 
...
$T=5$, $\tau=50$ and $R=3$ (Figure \ref{fig:power}). It evident that $LS$ 
method 
...
prediction. Also, GP and FIT methods which are single-locus methods, reveal 
