...
is the 
limited time for the experiment, which primarily depend on the organism's 
generation 
...
mainly designed 
to identify adaptation in pre/post fixations in static data.
In this article, we study the problem of identifying selection signatures in 
allele-frequency trajectories of small number of generations. 
We propose a Dynamic Composite Likelihood Ratio 
(DCLR) statistic for time-series data which computes its statistic by averaging 
likelihood ratios of each polymorphism in a sliding window. The likelihood of 
(neutral) and 
alternative (selection) hypotheses using the Wright-Fisher Markov chain model 
for each variant. Extensive simulation study shows that
DCLR achieves higher detection power methods on both soft and 
hard sweep simulations. 
Finally, we apply the DCLR statistic to the controlled experimental 
...
recombination event occurs, the whole genome is perfectly linked to the 
adaptive allele 
and "clonal interference" \cite{desai2007beneficial,lang2013pervasive} 
describes the adaptation process when more than one beneficial allele exist in 
the population.
...
Adaptation leaves a variety of signatures 
...
prevalence of long haplotypes 
	...
	traces of selective sweep in the polymorphism data.
In the experimental evolution with pooled-sequencing, only allele frequency 
of the population, and sometimes initial population's haplotypes are available. 

Traditionally, given static frequency data, Tajima's \emph{D} 
~\cite{tajima1989statistical}, 
Fay and Wu's \emph{H}~\cite{fay2000hitchhiking}, 
Composite Likelihood Ratio~\cite{nielsen2005genomic}, 
SFSelect~\cite{ronen2013learning} test statistics can be computed using the
...
background linkage in the genome.}. Despite their simplicity and intuitiveness, 
it has been shown that
  SFS-based tests often fail to distinguish demography from adaptation. They 
...
On the other hand, identifying selection in dynamic data is not well studied 
yet.
Feder et. al \cite{feder2014Identifying}  proposed Frequency Increment
 ...
 frequency/haplotype dynamic single-population data. Burke et 
 al.~\cite{burke2010genome} applied 
 ...
 Song et al. \cite{Terhorst2015Multi} computed LRT statistic by fitting 
 parameters to a Gaussian process model. Nevertheless, their detection power 
 under short experimental evolutions are yet to be known.
...
In this setting, constraints such as small population sizes, limited 
...
\cite{boyko2008assessing,desai2008polymorphism,sawyer1992population}. For 
example, dynamic data has been used to estimate model parameters 
...
Among different types of evolution experiments \cite{Barrick2013Genome} in 
this 
paper we only focus on adaptive evolution of multicellular sexual organisms 
with 
continues culture, fixed population size. For this setting, D. melanogaster is 
...
hypoxia tolerance \cite{zhou2011experimental} (300 generations),
...
extend it by a parametric formulation whcih they can estimate $s$ and 
compute likelihood.}.
