...
and SFSelect tests.
\begin{enumerate}[I.]
	\item {\bf Tajima D} As shown in the Appendix, dynamic of Tajima's D in hard 
	sweep is
	\beqq
	D_t=D_0+\log(1-\nu_t) \frac{W_0}{\log(2N)} -\nu_t^2 \Pi_0
	\eeqq
	where $\nu_t$ defined in term of $\nu_0$ and $s$ in \eqref{}, $W_0$ and 
	$\Pi_0$ are Watterson and Tajima estimate of $\theta$ at first generations.
	As shown in Figures \ref{fig:dynamic-weak} and \ref{fig:dynamic-strong} (last 
	row, left), the model is a reasonable. However in early generations, even for 
	strong selection, it is hard to discriminate between neutral evolution and 
	selective sweep. Moreover, it can only be used for the hard sweep, which is 
	makes it difficult to apply on real data.
	\item {\bf Fay Wu's H} As shown in the Appendix, dynamic of Fay Wu's H in 
	hard sweep is
	\beqq
	nH_t= \theta \nu_t \left(\frac{\nu_t+1}{2} - \frac{1}{(1-\nu_t)n+1}\right) +
	\theta (1-\nu_t)\left(\frac{n+1}{2n}-\frac{1}{(1-\nu_t)n+1}\right)
	\eeqq
	which is a good approximation only in strong selection, Figures 
...
where
\beq
	s&=\frac{2}{t}  \left( \eta(x_t)-\eta(x_0)\right)\\
	\frac{1}{ \eta(x_t)-\eta(x_0)}s&=\frac{2}{t} \ \ \ \forall t \in \Tc \\ 
	\eeq
we can put all together 
\beq
\bfa s = \bft\\
\bfa=\begin{bmatrix}
	\frac{1}{ \eta(x_{\tau_1})-\eta(x_0)}\\
	\vdots \\
	\frac{1}{ \eta(x_{\tau_T})-\eta(x_0)}
\end{bmatrix}, \ \ \ \ \ 
\bft=\begin{bmatrix}
	\tau_1-\tau_0\\
	\vdots \\
...
\frac{\sum_r\sum_t  \frac{\tau_t-\tau_0}{\eta(x_{r,t})-\eta(x_{r,0})}}
{\sum_r\sum_t\left(  \frac{1}{ \eta(x_{r,t})-\eta(x_{r,0})}\right)^2}
\eeq
\newpage
Basically
\beq
...
\newpage
	...
	$f''(X)=-\frac{1}{X^2}$
	\beq
	\Ebb[\log(X)]&\approx \log(\mu_X)- \frac{\sigma_X^2}{2\mu_X^2}\\
	\Var[\log(X)]&\approx \frac{\sigma_X^2}{\mu_X^2}
	\eeq
...
cite
Strength of Selection Pressure Is an Important Parameter Contributing to the 
Complexity of Antibiotic Resistance Evolution
