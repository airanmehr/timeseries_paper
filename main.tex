...
Depending on the 
value of allele frequency at the time selection started, 
selective sweep is 
often 
categorized in two main regimes: hard and soft sweep, Figure 
\ref{fig:sweep}. 
Hard sweep occurs when the initial allele (carrier) frequency 
$\nu_0=1/N_e$ is 
at 
minimum, and soft-sweep otherwise. This small difference 
could have nontrivial
...
those of neutral evolution and demography.
 ...
 different levels of 
 detail. The very first question is to ask whether a region 
 (e.g. a window of 
...
$\nu_t$ at a site 
Brownian motion approximation considers future allele 
frequencies to be drawn 
...
thus most of the 
sites are at low frequencies. Thus if the selecting site is 
chosen randomly 
...
On the other hand, alternative models for single-locus 
selection can be 
...
SFS is a measure 
of diversity in a genomic region and reduction of diversity 
is a signal of 
selection. Detecting selection based on the reduction in 
genomic diversity is a 
subtle 
task, because 
\begin{enumerate}[(i)]
...
one, which makes it a convex combination, aka discrete 
expectation. For 
example, 
test statistics for Tajima's $D$\cite{tajima1989statistical}, 
Fay Wu's 
$H$\cite{fay2000hitchhiking3d} and 
SFSelect\cite{ronen2013learning} can be obtained by a dot 
product of SFS vector 
with their weight vector.

Whether the dynamic of SFS in times-series can improve the 
detection power in 
...
On the other 
hand, scalar test statistics such as $D$, $H$ and SFSelect 
are easier to 
analyze and visualize.

In the simplest case, statistical test can be designed to 
test whether the 
...
intuition how does SFS statistics behave under different 
...
\begin{enumerate}[I.]
	...
	\eqref{eq:nut}, we 
	can use \eqref{eq:tdt} to detect the selection and 
	estimate the strength of 
	selection at the same time by performing a nonlinear 
	regression to find 
	$s$. A likelihood ratio, similar to \eqref{eq:lrt}, 
	...
	In addition, in derivation of \eqref{eq:tdt} it has been 
	assumed that the 
	population undergoes a hard-sweep, and the model becomes 
	invalid as the 
	initial carrier frequency deviates from that of 
	hard-sweep, Figure 
	\ref{fig:dynamic-weak} and \ref{fig:dynamic-strong} (last 
	row, right).
	
	Finally, in a infinite population size setting, 
	differentiating the between 
	selection and drift is difficult in early generations of 
	hard sweep.
	More precisely, w.l.o.g. when $D_0=0$ and $\Pi_0=W_0=1$, 
	$D_t$ is sum of a 
	logarithmic and a squared term, blue and green curves in 
	Figure 
	\ref{fig:tdterms}. As shown in the Figure 
	\ref{fig:tdterms} right, in early 
	generations of sweep where carrier frequency is low, 
	$D_t$ is either 
