...
\maketitle
\begin{abstract}
Experimental evolution studies are powerful tools for observing
...
next-generation sequencing which made it possible to study the genetic 
trajectory of populations at different generations. Methods for inferencing from
time-series genomic data are under active development.  
In this research, we study the problem of identifying selection in short amount 
of time using classical polymorphism summary statistics such as
Tajima's $D$ and Fay Wu's $H$ and propose a new statistic which outperforms 
existing methods on extensive simulations of soft and hard sweep regimes. 
Finally we apply the 
proposed statistic to the controlled experimental evolution of D. melanogaster 
...
genomic region\cite{vitti2013detecting}. This approach is called Composite 
Likelihood Ratio Test (CLRT) \cite{williamson2007localizing} which effectively 
makes more robust predictions for detecting selection in a genomic region. We 
implement CLRT criteria by taking average of top one percentile of the 
likelihood ratios in a window.
