...
\label{eq:pihmm}
\end{equation}
To clarify the notation, we use \comale-M to denote Markov Chain based
likelihood scores, and \comale-H to denote HMM based likelihood
scores. As short-hand notation, we use $M$, Eq.~\ref{eq:mcts}
(respectively, $H$, Eq.~\ref{eq:hmmml}) to denote the \comale\ scores
for individual variants. Similarly, we use $\Mc_{\pi}$ (respectively,
$\Hc_{\pi}$) to denote the composite \comale\ scores for a genomic
region. Note that ${\cal M}_{100},{\cal H}_{100}$ (respectively,
